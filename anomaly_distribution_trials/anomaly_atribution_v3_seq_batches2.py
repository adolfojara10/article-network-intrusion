...
#   - save row results CSV + 2x2 confusion matrix CSV
...
# Check if the test data has already been saved from a previous run
if os.path.exists(TEST_DATA_CSV) and os.path.exists(TRAIN_DATA_CSV):
    print(f"[INFO] Test and train datasets found. Loading data from disk...")
    train_df = pd.read_csv(TRAIN_DATA_CSV)
    test_df = pd.read_csv(TEST_DATA_CSV)
    print(f"[INFO] Loaded train size: {len(train_df)} | Loaded test size: {len(test_df)}")
else:
    print("[INFO] Datasets not found. Performing train/test split and saving data...")
    ...
    # Save the datasets to a CSV file for reproducibility and resuming
    test_df.to_csv(TEST_DATA_CSV, index=False)
    train_df.to_csv(TRAIN_DATA_CSV, index=False)
    print(f"[INFO] Saved train data to {TRAIN_DATA_CSV}")
    print(f"[INFO] Saved test data to {TEST_DATA_CSV}")
...
# --- MODIFIED PART: RESUME FROM LAST CHECKPOINT ---
START_ROW_IDX = 0
if os.path.exists(RESULTS_CSV) and os.path.getsize(RESULTS_CSV) > 0:
    print(f"[INFO] Found previous results file. Resuming classification...")
    existing_results_df = pd.read_csv(RESULTS_CSV)
    if not existing_results_df.empty:
        # Get the index of the last row processed
        last_processed_idx = len(existing_results_df) - 1
        # Start from the next row (not next batch, to avoid re-processing)
        START_ROW_IDX = last_processed_idx + 1
        print(f"[INFO] Resuming from row {START_ROW_IDX}")
    else:
        print("[INFO] Previous results file is empty. Starting from row 0.")
else:
    print("[INFO] No previous results file found. Starting a new classification run.")

# --- BATCH PROCESSING LOOP ---
t0 = time.time()
# The range now starts from START_ROW_IDX
for i in tqdm(range(START_ROW_IDX, len(test_X), BATCH_SIZE), desc="Classifying batches"):
    ...
    # Prepare the results for the current batch
    batch_results = []
    ...
    # Save the batch results incrementally
    batch_results_df = pd.DataFrame(batch_results)
    batch_results_df.to_csv(RESULTS_CSV, mode='a', index=False, header=not os.path.exists(RESULTS_CSV))

elapsed = time.time() - t0
print(f"[INFO] Done in {elapsed:.2f}s  ({(len(test_X) - START_ROW_IDX) / max(elapsed,1e-9):.1f} rows/sec).")


# ============================ SAVE FINAL CONFUSION MATRIX ============================
print("\n[INFO] Loading all results to calculate final metrics...")
if not os.path.exists(RESULTS_CSV):
    print("[INFO] No results to load. Exiting.")
else:
    results_df = pd.read_csv(RESULTS_CSV)
    print(f"[INFO] Loaded {len(results_df)} results from {RESULTS_CSV}")
