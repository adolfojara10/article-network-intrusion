...
def classify_row_by_anomaly_score(
    ...
    for t in T_ben:
        ...
        #print(f"Debug: Target={t}, Attr={attr}, Vec={vec}, TotalScore_B={total_score_B}")
    ...
    for t in T_mal:
        ...
        #print(f"Debug: Target={t}, Attr={attr}, Vec={vec}, TotalScore_M={total_score_M}")
    ...
    print(f"Debug: Final TotalScore_B={total_score_B}, TotalScore_M={total_score_M}, Label={label}")
...
t0 = time.time()
G_benign = visualize_causal_graph(adj_ben, variable_names=feature_names, threshold=0.6)
G_malicious = visualize_causal_graph(adjacency_matrix=adj_mal, variable_names=feature_names, threshold=0.6)
...
T_ben = topk_influenced_targets_from_adj(adj_ben, k=K_REF, threshold=0.6)
T_mal = topk_influenced_targets_from_adj(adj_mal, k=K_REF, threshold=0.6)
print("    T_ben:", T_ben)
print("    T_mal:", T_mal)
...
indices = list(range(10))  # Only first 100 rows for testing
...
with ctx.Pool(processes=n_proc) as pool:
    results = list(tqdm(pool.imap_unordered(_classify_row_index, indices), total=len(indices), desc="Classifying rows"))
